...
export function toPayload(m: Metrics, label?: string, fromPage = false): Payload {
  return { v: 1, from: m.from, to: m.to, series: m.series, aux: m.aux, stats: m.stats, ...(m.quotes?.length ? { quotes: m.quotes } : {}), ...(m.coverage ? { cov: [m.coverage.from, m.coverage.to] as [string, string] } : {}), ...(label ? { label } : {}), ...(fromPage ? { e: 1 as const } : {}) };
}
